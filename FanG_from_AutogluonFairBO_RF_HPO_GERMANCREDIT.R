...
if(.Platform$OS.type=="unix") {
  ...
  autogluonFairBO_results_folder = "/home/ac21041/Desktop/autogluon-0.3.1/FanG-HPO_AutogluonFairBO_runs/RF_HPO_GERMANCREDIT_results"
  conda_python("autogluonFairBO")
} else {
  use_python(python="C:/Users/Public/anaconda3",required=T)
  autogluonFairBO_results_folder = "G:/Il mio Drive/AutogluonFairBO_results/RF_HPO_GERMANCREDIT_results"
  conda_python("py3.8")
}
...
source.1 = function( x ) {
  ...
  py_run_string("dataset_name = 'GERMANCREDIT_full'")
...
source.2 = function( x ) {
  ...
  py_run_string("dataset_name = 'GERMANCREDIT_redux'")
  py_run_string("sensitive_features = ['Gender.Female']")
  py_run_string("target = 'has.bad.credit.risk'")
...
for( f in files ) {
  ...
  if( !dir.exists("results") )
    dir.create("results")
  saveRDS( object=run.info, file=paste0("results/results_GERMANCREDIT_RF_file_",run,".RDS") )
  cat(" done!\n")
